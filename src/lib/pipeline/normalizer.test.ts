...
describe('normalize', () => {
  describe('field transformations', () => {
    it('[P1] should normalize title to title case', () => {
    ...
    it('[P1] should trim title and company', () => {
      const { normalized } = normalize([
    ...
    it('[P1] should preserve salary values from adapter', () => {
      const { normalized } = normalize([
    ...
    it('[P1] should handle missing salary', () => {
      const { normalized } = normalize([
    ...
    it('[P1] should preserve description_html unchanged', () => {
    ...
    it('[P1] should sanitize description_text for XSS', () => {
      const { normalized } = normalize([
        createRawListing({
    ...
    it('[P1] should strip event handlers from description_text', () => {
      const { normalized } = normalize([
        createRawListing({
    ...
    it('[P1] should set discoveredAt to current timestamp', () => {
      const before = new Date()
      const { normalized } = normalize([createRawListing()])
    ...
    it('[P1] should set pipelineStage to discovered', () => {
      const { normalized } = normalize([createRawListing()])
      expect(normalized[0].pipelineStage).toBe('discovered')
    })

    it('[P1] should preserve sourceUrl and applyUrl', () => {
      const { normalized } = normalize([
        createRawListing({
  ...
  describe('is_remote three-state contract', () => {
    it('[P1] should preserve explicit is_remote true from adapter', () => {
      const { normalized } = normalize([
    ...
    it('[P1] should preserve explicit is_remote false from adapter', () => {
      const { normalized } = normalize([
    ...
    it('[P1] should derive is_remote from location when undefined', () => {
      const { normalized } = normalize([
    ...
    it('[P1] should set is_remote to undefined when location is unknown', () => {
      const { normalized } = normalize([
  ...
  describe('fingerprint generation', () => {
    it('[P1] should generate same fingerprint for same title+company+location', () => {
      const { normalized: [a] } = normalize([createRawListing({ external_id: 'a' })])
      const { normalized: [b] } = normalize([createRawListing({ external_id: 'b' })])
      expect(a.fingerprint).toBe(b.fingerprint)
    })

    it('[P1] should generate different fingerprints for different titles', () => {
      const { normalized: [a] } = normalize([
        createRawListing({ title: 'Software Engineer', external_id: 'a' }),
      ])
      const { normalized: [b] } = normalize([
  ...
  describe('within-batch dedup', () => {
    it('[P1] should remove fingerprint-identical listings within same batch', () => {
      const { normalized, skippedCount } = normalize([
    ...
    it('[P1] should keep listings with different fingerprints', () => {
      const { normalized } = normalize([
  ...
  describe('source attribution', () => {
    it('[P1] should populate sources array with single source', () => {
      const { normalized } = normalize([createRawListing()])
  ...
  describe('benefits extraction', () => {
    it('[P1] should extract benefits from description text', () => {
      const { normalized } = normalize([
        createRawListing({
          description_text: 'We offer health insurance, 401k matching, and unlimited pto.',
        }),
      ])
      expect(normalized[0].benefits).toBeDefined()
      expect(normalized[0].benefits).toContain('health insurance')
      expect(normalized[0].benefits).toContain('401k')
      expect(normalized[0].benefits).toContain('unlimited pto')
    })

    it('[P1] should return undefined when no benefits found', () => {
      const { normalized } = normalize([
        createRawListing({
          description_text: 'Build amazing software products.',
        }),
  ...
  describe('searchText population', () => {
    it('[P1] should populate searchText from title + company + description', () => {
      const { normalized } = normalize([createRawListing()])
  ...
  describe('edge cases', () => {
    it('[P1] should handle empty input array', () => {
      const { normalized, skippedCount } = normalize([])
      expect(normalized).toEqual([])
      expect(skippedCount).toBe(0)
    })

    it('[P1] should handle multiple valid listings from different sources', () => {
      const { normalized } = normalize([
    ...
    it('[P2] should strip javascript: URIs from description_text', () => {
      const { normalized } = normalize([
        createRawListing({
